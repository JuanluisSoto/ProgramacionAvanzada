...
void ATwinStickNPC::ProjectileImpact(const FVector& ForwardVector, float InDamageMultiplier)
{
	...
	// award points (multiplied by damage boost if active)
	if (ATwinStickGameMode* GM = Cast<ATwinStickGameMode>(GetWorld()->GetAuthGameMode()))
	{
		GM->ScoreUpdate(FMath::RoundToInt32(Score * InDamageMultiplier));
	}
